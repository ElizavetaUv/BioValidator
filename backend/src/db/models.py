...
from sqlalchemy import Column, ForeignKey, Integer, Sequence, Text, UniqueConstraint
...
class Mutation(Base):
    ...
    __table_args__ = (
        # UniqueConstraint("hugo_symbol", "variant_type", "reference_allele", "chromosome", "start_position", "end_position"),
        {"schema": "main"},
    )
...
class Metric(Base):
    ...
    __table_args__ = (
        UniqueConstraint("name", "version", "value", "sample_id"),
        {"schema": "main"},
    )
    ...
    sample: Mapped["Sample"] = relationship("Sample", back_populates="metrics", uselist=False)
...
class Sample(Base):
    ...
    reference: Mapped["Reference"] = relationship("Reference", back_populates="samples", uselist=False)
...
class Reference(Base):
    ...
    samples: Mapped[List["Sample"]] = relationship("Sample", back_populates="reference")
